...
const JSON_FIELDS = {
  goal: '{"label":"不超过14字的标题（给这个前置目的起个名）","principle":"达成该目的必须先达成的那个前置目的（一句话大白话，用「先要…」开头）","reasoning":"为什么必须先做到它（1~2句大白话，讲清楚因果）","keywords":["3~6个关键词"]}',
  text: '{"label":"不超过14字的标题","essence":"用大白话提炼他真正想要什么（1~2句）","principle":"达成这个目的必须先达成的那个前置目的（一句话大白话，用「先要…」开头）","reasoning":"为什么必须先做到它（1~2句大白话）","keywords":["3~6个关键词"]}',
  derive: '{"label":"不超过14字的标题","principle":"更前置的那个目的（一句话大白话，用「先要…」开头）","reasoning":"为什么必须先做到它（1~2句大白话）","keywords":["3~6个关键词"]}',
};
...
function buildDeriveMessages({ text, mode, context, hint }) {
  ...
  const tasks = {
    ...
    derive: `用户给出了一条【前置目的】，需要继续往前拆（当前为第 ${context && context.depth != null ? context.depth + 1 : 1} 层）：\n【待拆解】${text}\n${context && context.ancestors && context.ancestors.length
      ...
      : ''}请继续追问：要达成这个目的，最基本、最关键、更前置的目的又是什么？给出下一层。\n要求：给的是一个"可以达成的目的/条件"，不是抽象理论或定律；不要重复已走过的链条。${hintLine}`,
  };
  return [
